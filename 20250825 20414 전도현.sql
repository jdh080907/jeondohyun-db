...
round((patio_to_report(sum(sal)) over (partition by deptno))*100,2)"%"
from emp group by deptno,ename,sal
order by 1;

--11) loan 테이블을 사용하여 1000번 지점의 대출 내역을 출력하되 대출일자, 대출 종목코드, 대출건수, 대출 총액, 누적 대출금액을 아래와 같이 출력하세요.
select from loan where l_store=100 and l_code=100;
select l_date"대출일자",l_code 대출종목코드,l_qty대출건수.l_total 대출금액,
sum(l_total)over(order by l_total)누적대출금액
from loan where 1_store=1000;
