...
--1. 학생 테이블 (student) 과 학과 테이블 (department) 테이블을 사용하여 학생이름, 1 전공학과번호(deptno1) , 1전공 학과 이름을 출력하세요.
 select name stu_name,deptno1,dname
 dept_name
...
select name, e2.position, pay, s_pay
low_pay, e_pay high_pay
...
from emp2 e2, p_grade p
where
trunc(2) between p.s_age and p.e_age;
...
select p1.profno, p1.name, p1.hiredate, count(nvl2(p2.profno,p1.profno,null))
as count
