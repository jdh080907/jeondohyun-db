...
select s.name "SYU_NAME", p.name "PROF_NAME" from student S, professor p where s.profno(+) = p.profno union
select s.name "STU_NAME", p.name "PROF_NAME" from student S, professor P where S.profno = p.profno(+);
...
--1. 학생 테이블 (student) 과 학과 테이블 (department) 테이블을 사용하여 학생이름, 1 전공학과번호(deptno1) , 1전공 학과 이름을 출력하세요.
 ...
 select name syu_name,deptno1, dname dept_name from student s, department d where d.deptno=s.deptno1;
...
from emp2 e2, p_grade p where e2.position=p.position;
...
from emp2 e2 , p_grade p where
trunc(months_between(sysdate,e2.birthday)/12) between p.s_age and p.e_age;
